		...
		constructor(options) {
					...
					else 
					{
						...
						if(subMenu)
						{
							...
							button.on("mouseenter", (evt) => { 
								...
								menu.find('[data-ctxmenu-sub]').filter(o => o != subMenu[0]).fadeOut(50);
							...
							button.on("mouseleave", (evt) => { 
								...
								if(!$$.mouseIntersects(subMenu, evt))
								{
			...
			$$(options.element).on('contextmenu', (e) => {
				...
				if (!_preventDefault) { 
					...
					if((e.pageY + menuHeight) > $$.screen.dimentions().height)
					{
