 ...
 * @Dependencies: inQuiry.js, inQuiry.animation.js, inQuiry.objectCookie.js, inQuiry.toolTip.js, inQuiry.ui.js, inQuiry.contextMenu.js, (//cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css)
 */
(function ($$) {
	"use strict";
	...
	$$.desktopWindows = {
		...
		setUpDesktop: function (selector) {
			...
			this.add = function(options) {
				...
				$$(".desktop-desktop").each((i, desktop) => {
					...
					subMenuBuild.push({
						command: "Open on desktop " + (parseInt(desktop.attr("id").replace("desktop", "")) + 1), 
				...
				abs.ContextMenu({
					...
					bindings: [
						...
						},
						{
							...
							subIconClass: "fa fa-arrow-right",
        ...
        setUpShield: function (selector, backcolor, transparency) {
            ...
            this.resize = function () {
                ...
                if (this.element.style.visibility == '') {
					...
					var r = $$.screen.dimentions();

					this.element.style.width = r.width + 'px';
                    
					if (($$.browser.firefox) || ($$.browser.msie) || ($$.browser.mozilla)) { 
        ...
        window: function (options) {
			...
			this.load = function () {
                ...
                if ((this.defaults.position & $$.desktopWindows.STATIC) != $$.desktopWindows.STATIC) {
					...
					var ca = new $$.cookie.create('desktopWindows_' + this.windowframe.id, null);
                   
					if (ca.read() == null) {
                        
						ca = new $$.cookie.create('desktopWindows_' + this.windowframe.id, 365);

						var pos = $$(this.windowframe).cssTranslateValues();

                        ca.keys.add(new $$.cookie.item('x', pos.x + "px"));

                        ca.keys.add(new $$.cookie.item('y', pos.y + "px"));

                        ca.keys.add(new $$.cookie.item('w', this.windowframe.style.width));

                        ca.keys.add(new $$.cookie.item('h', this.windowframe.style.height));
									...
									callback: (buttonItem, menuItem) => {
										...
										$$.desktopWindows.serviceappsbox.add({
											...
											action: () => {
												...
												$$.desktopWindows.serviceappsbox.appLinks = $$.desktopWindows.serviceappsbox.appLinks.grep(h => h != button[0]);
												button.remove(); 
											}
										});
				...
				try {
                    ...
                    if (winForm.defaults.onminimize != null && winForm.defaults.windowicon != null) {
						...
						if(winForm.defaults.appType == $$.desktopWindows.APPSERVICE) {
							...
							$$.desktopWindows.serviceappsbox.appLinks = $$.desktopWindows.serviceappsbox.appLinks.grep(h => h != serviceButton[0])
						...
						if(winForm.defaults.appType == $$.desktopWindows.APPSETTING) {
							...
							$$.desktopWindows.settingsappsbox.appLinks = $$.desktopWindows.settingsappsbox.appLinks.grep(h => h != settingButton[0])
        ...
        close: function (windowId) {
			...
			var ca = new $$.cookie.create('desktopWindows_' + p.windowframe.id, null);
            ...
            ca = new $$.cookie.create('desktopWindows_' + p.windowframe.id, 365);
            ...
            ca.keys.add(new $$.cookie.item('w', document.getElementById(p.windowframe.id).style.width));

            ca.keys.add(new $$.cookie.item('h', document.getElementById(p.windowframe.id).style.height));
				...
				try {
					...
					if (selected.defaults.onminimize != null && selected.defaults.windowicon != null) {
						...
						if(selected.defaults.appType == $$.desktopWindows.APPSETTING) {
							...
							$$.desktopWindows.settingsappsbox.settingLinks = $$.desktopWindows.settingsappsbox.settingLinks.grep(h => h != settingButton[0])
					...
					$$.desktopWindows.openwindows = $$.desktopWindows.openwindows.grep((item, idx) => {
